"""
Knowledge Base — read-only helpers for agents and tools.

All public functions return dicts in exactly the same format that the old JSON
fixture files used, so agent code can continue to consume them unchanged.
"""
...
def get_kb_materials() -> dict:
    """Return ``{"papers": [...], "stock_items": [...], "finishes": [...], "adhesives": [...]}``."""
    with get_connection() as conn:
...
def get_kb_operations() -> dict:
    """Return ``{"operations": [...], "product_type_routes": {product_type: [op_id, ...]}}``."""
    with get_connection() as conn:
...
def get_game_components() -> list[dict]:
    """Return the catalog of purchasable board-game components."""
    with get_connection() as conn:
        try:
            rows = conn.execute(
...
def get_cost_rates_by_category() -> dict[str, dict[str, float]]:
    """Nested ``{category: {rate_key: value}}`` dict for the cost calculator."""
    with get_connection() as conn:
        try:
            rows = conn.execute(
